...
public class LoginFrame extends Frame implements ActionListener, Mediator {
    ...
    public LoginFrame(String title) {
        ...
        // 레이아웃 매니저를 사용해 5×2 그리드를 만든다 (마지막 행에 statusLabel 배치)
    ...
    public void colleagueChanged() {
        ...
        // OK 버튼의 활성화 상태에 맞추어 하단 라벨의 텍스트와 색상을 동기화
    ...
    private void userpassChanged() {
        // 과제 명세서 조건: Username이 4자 이상이어야 비밀번호 창이 열림
        if (textUser.getText().length() >= 4) {
            textPass.setColleagueEnabled(true);
            
            // Password도 4자 이상이어야 로그인(OK) 버튼이 활성화됨
            ...
            // Username이 4자 미만이면 Password 창을 잠그고 OK 버튼도 비활성화
            textPass.setColleagueEnabled(false);
            buttonOk.setColleagueEnabled(false);
        }
    }
